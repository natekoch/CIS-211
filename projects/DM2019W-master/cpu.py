...
class ALU(object):
    ...
    def exec(self, op: OpCode, in1: int, in2: int) -> Tuple[int, CondFlag]:
        try:
            ...
            if result == 0:
                result = (result, CondFlag.Z)
            elif result> 0:
                result = (result, CondFlag.P)
            elif result < 0:
                result = (result, CondFlag.M)
        except:
            result = (0, CondFlag.V)
        return result
...
class CPU(MVCListenable):
    ...
    def step(self):
        ...
        if self.condition & instr.cond:
            ...
            result = self.alu.exec(instr.op, self.registers[instr.reg_src1].get(), 
            (instr.offset + self.registers[instr.reg_src2].get()))
